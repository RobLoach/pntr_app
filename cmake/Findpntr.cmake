...
# TODO: function should just pull deps (no arg-parsing) if PNTR_APP_WINDOW_* and PNTR_APP_SOUND_* are already set
...
function(add_pntr target)
    ...
    if (${ARGC} GREATER 1)
        ...
        foreach(a IN LISTS pntr_args)
            ...
            elseif ("${a}" STREQUAL "RAYLIB_DRM")
                set(PNTR_APP_WINDOW "RAYLIB")
                # TODO: add define for raylib build
            elseif ("${a}" STREQUAL "SDL2")
                set(PNTR_APP_WINDOW "SDL2")
            elseif ("${a}" STREQUAL "SDL2_SOUND")
                set(PNTR_APP_SOUND "SDL2")
    ...
    if(NOT DEFINED PNTR_APP_SOUND)
        ...
        if ("${PNTR_APP_WINDOW}" STREQUAL "SDL2")
            set(PNTR_APP_SOUND "SDL2")
        endif()
    ...
    if(NOT DEFINED PNTR_APP_WINDOW)
        ...
        if ("${PNTR_APP_SOUND}" STREQUAL "SDL2")
            set(PNTR_APP_WINDOW "SDL2")
        endif()
    endif()
    ...
    # tidy up matching window/sound
    string(REPLACE "raylib_raylib" "raylib" pntr_lib_name "${pntr_lib_name}")
    string(REPLACE "sdl2_sdl2" "sdl2" pntr_lib_name "${pntr_lib_name}")
    ...
    message(PNTR_APP_LOCATION-before ": ${PNTR_APP_LOCATION}")

    # this allows use to override where pntr_app is located
    if ("${PNTR_APP_LOCATION}" STREQUAL "")
        ...
        set(PNTR_APP_LOCATION "${fpntr_SOURCE_DIR}")
    endif()

    # this allows user to override their pntr_app entrypoint file
    if("${PNTR_APP_ENTRY}" STREQUAL "")
        set(PNTR_APP_ENTRY "${PNTR_APP_LOCATION}/include/pntr_app.c")
    endif()

    message(PNTR_APP_LOCATION ": ${PNTR_APP_LOCATION}")
    message(PNTR_APP_ENTRY ": ${PNTR_APP_ENTRY}")
    ...
    if (NOT TARGET "${pntr_lib_name}")
        add_library("${pntr_lib_name}" STATIC "${PNTR_APP_ENTRY}")
        set_property(TARGET "${pntr_lib_name}" PROPERTY C_STANDARD 11)

        # TODO: this maps to older variables. Still need to set them up for newer style defines
    ...
    include_directories("${PNTR_APP_LOCATION}/include")
    ...
    if ("${PNTR_APP_SOUND}" STREQUAL "SDL2" OR "${PNTR_APP_WINDOW}" STREQUAL "SDL2")
